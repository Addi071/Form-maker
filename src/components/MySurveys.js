...
function MySurveys() {
    ...
    const fetchSurveys = async () => {
      try {
        ...
        for (const survey of data) {
          const resCount = await fetch(`${config.BACKEND_URI}/api/responses/${survey._id}/count`,
            {
              credentials: "include",
            }
          );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${config.BACKEND_URI}/survey/${id}`, {
        method: "DELETE",
      },
    {
    credentials: "include",
  ...
  const handleDownloadCSV = (surveyId) => {
    window.open(`${config.BACKEND_URI}/api/responses/${surveyId}/download`, "_blank");
  };
